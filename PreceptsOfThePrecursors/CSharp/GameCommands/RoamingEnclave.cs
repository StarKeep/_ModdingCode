    ...
    public class ActivatePlayerRoamingYoungling : BaseGameCommand
    {
        public override void Execute( GameCommand command, ArcenSimContext context )
        {
            ...
            if ( enclaveConstructor == null )
                return;
            if (enclaveConstructor.FleetMembership.ExplicitBaseSquadCap <= 0 )
            {
            ...
            enclave.FleetMembership.Fleet.Category = FleetCategory.PlayerMobile;
            enclave.FleetMembership.Fleet.NameRaw = "Pearl Enclave's Gift";      
        }
    }
